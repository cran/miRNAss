...
The hairpin sequences can be extracted from a raw genome using HExtractor
...
\url{http://fich.unl.edu.ar/sinc/blog/web-demo/mirnafe-full/}). The whole
pipeline consist on extract the hairpins from a raw genome in fasta format.
The output, also in fasta format is the input to miRNAfe, which as result gives
a comma separate file that can be easily load into R.
